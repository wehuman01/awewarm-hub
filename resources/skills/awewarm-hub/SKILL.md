...
- Invite codes (`awi_...`) are one-time secrets (48 h by default). Anyone who can read the data dir — or sees the code — can use a pending invite.
- Restarting `serve` is safe by design: no secrets live on disk; each user's machine re-claims and re-pushes its keys on next contact.
...
2. Invite codes are one-time secrets: hand them out promptly, over a private channel; `revoke awi_...` the moment one leaks.
