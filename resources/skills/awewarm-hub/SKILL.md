...
| Admin — run on request | `awewarm-hub invite [--name <who>] [--count N] [--expires-in 30m|12h|7d] [--machines N]` (mints one-time codes; `--count` batch-mints codes that share name, expiry, and machine cap), `awewarm-hub invite revoke <awi_...>` (kill an invite: pending stops pairing, used suspends its tenant — reversible), `awewarm-hub invite revoke <awi_...> --delete` (wipe the ledger row outright; a used one takes its tenant — irreversible, confirm first), `awewarm-hub invite restore <awi_...>` (undo), `awewarm-hub invite rename <awi_...> <name>` (relabel an invite's note; a used one's tenant follows), `awewarm-hub config --max-tenants/--max-conns-per-tenant/--max-machines N [--reset]` (capacity caps; a running serve adopts them without a restart), `awewarm-hub config --data-dir /data [--unset]`, `awewarm-hub self-update` |
...
| "Wipe this invite from the ledger for good", "彻底删除邀请码/清理台账" | `awewarm-hub invite revoke <awi_...> --delete` — no revoked tombstone stays; a used one takes its tenant (token dead, slot freed, workspace kept on disk), `restore` cannot bring it back. Destructive: confirm the user truly wants deletion, not the reversible `revoke` (suspension). |
| "An invite leaked / was sent to the wrong person", "邀请码泄露" | `awewarm-hub invite revoke awi_...` kills a pending code on the spot; a used one suspends the tenant it produced. |
...
| "The note on an invite is wrong", "改备注/重命名" | `awewarm-hub invite rename <awi_...> <name>` — relabels the invite; a used one's tenant follows so `list users` shows the new name too. Non-destructive: code, status, caps, expiry untouched. |
...
| "Hub is full", "容量不够" | `awewarm-hub config --max-tenants 20` — a running serve adopts it without a restart; or restart `serve` with higher flags, or free a slot by suspending a tenant (`revoke`, active tenants only). `--max-conns-per-tenant` likewise. |
| "Raise/lower the capacity caps", "调整容量上限" | `awewarm-hub config` shows what's in effect and where from; `--max-tenants/--max-conns-per-tenant/--max-machines N` retune (adopted live), `--reset` clears back to the 10/5/1 defaults. `--max-machines` is the default stamped into each NEW invite — existing codes keep their own cap. |
...
- Capacity lives in the registry's serve record — one truth for every process: serve flags stamp it at launch, `config --max-*` retunes it at any time, and a running serve adopts the new values without a restart (every tenant action and each tick re-read the record). `--max-tenants` (default 10, active tenants only), `--max-conns-per-tenant`, `--max-machines` (the default stamped into each new invite; `invite --machines N` overrides per code). A light per-tenant rate limit (60 requests/minute) stops a looping client.
...
awewarm-hub invite revoke awi_...   # pending stops pairing; used suspends its tenant
awewarm-hub invite restore awi_...  # undo; refuses when the hub is full
awewarm-hub invite revoke awi_... --delete  # wipe the row outright (irreversible; a used one takes its tenant, workspace kept)
```

## Core Rules

1. Never run `awewarm-hub serve` from an agent session — it is a resident process the operator owns.
2. Invite codes are one-time secrets and tenant tokens are live credentials: hand either out promptly, over a private channel; `invite revoke awi_...` the moment a code leaks.
