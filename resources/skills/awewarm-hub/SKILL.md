...
| "The note on an invite is wrong", "改备注/重命名" | `awewarm-hub rename <awi_...> <name>` — relabels the invite; a used one's tenant follows so `list users` shows the new name too. Non-destructive: code, status, caps, expiry untouched. |
